...
Then(
    /^SmartThings (Ceiling Fan Light|Ceiling Fan|Office Dimmer|Office Light|Office Fan|Entryway Lights|Kitchen Light|Bathroom Light|Bedroom Lamp|Bedroom Fan|Bedroom Light|Porch Light|Outside Light|Outdoor Lights) button is in (On|Off|Low|Medium|High) state$/,
    (buttonName, state) => {
        switch (buttonName) {
...
Then(
    /^SmartThings (Ceiling Fan Light|Office Dimmer|Office Light|Kitchen Light) dimmer value is (\w+) on SmartThings page$/,
    (dimmerName, value) => {
        switch (dimmerName) {
            case 'Ceiling Fan Light':
            ...
            case 'Office Light':
                ...
                break;
            case 'Kitchen Light':
                expect(SmartThingsPage.getTheaterTab().kitchenLightDimm.getAttribute('value')).toEqual(
                    value.toString()
                );
                break;
            default:
...
Then(
    /^SmartThings (Entryway Lights|Ceiling Fan|Office Fan) is (\w+) and (\w+) on SmartThings page$/,
    (dimmerName, state, value) => {
        switch (dimmerName) {
            case 'Entryway Lights':
                switch (state) {
                    case 'on':
                        expect(SmartThingsPage.getTheaterTab().entrywayLightsOnButton.getProperty('checked')).toEqual(
                            true
                        );
                        break;
                    case 'off':
                        expect(SmartThingsPage.getTheaterTab().entrywayLightsOffButton.getProperty('checked')).toEqual(
                            true
                        );
                        break;
                    default:
            ...
            case 'Ceiling Fan':
                if (state === 'off') {
                    ...
                    if (parseInt(value) <= 33) {
                        expect(SmartThingsPage.getTheaterTab().ceilingFanLowButton.getProperty('checked')).toEqual(
                            true
                        );
                    }
                    if (parseInt(value) > 33 && parseInt(value) <= 66) {
                        expect(SmartThingsPage.getTheaterTab().ceilingFanMediumButton.getProperty('checked')).toEqual(
                            true
                        );
                    }
                    if (parseInt(value) > 66) {
                        expect(SmartThingsPage.getTheaterTab().ceilingFanHighButton.getProperty('checked')).toEqual(
                            true
                        );
                    }
                }
            ...
            case 'Office Fan':
                if (state === 'off') {
                    ...
                    if (parseInt(value) > 33 && parseInt(value) <= 66) {
                        expect(SmartThingsPage.getTheaterTab().officeFanMediumButton.getProperty('checked')).toEqual(
                            true
                        );
                    }
                    if (parseInt(value) > 66) {
                        expect(SmartThingsPage.getTheaterTab().officeFanHighButton.getProperty('checked')).toEqual(
                            true
                        );
                    }
                }
...
Then(
    /^SmartThings (Ceiling Fan Light|Office Dimmer|Office Light|Kitchen Light) dimmer value is (\w+) on Dashboard page$/,
    (dimmerName, value) => {
        switch (dimmerName) {
            case 'Ceiling Fan Light':
                expect(DashboardPage.theaterCeilingFanLightButton.getText()).toEqual(
                    `${dimmerName}\n${value.toString()}%`
                );
                break;
            case 'Office Dimmer':
