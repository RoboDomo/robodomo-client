...
When(
    /^User clicks on (New York, NY|Los Angeles, CA) tab$/,
    { wrapperOptions: { retry: 2 } },
    menuButton => {
        switch (menuButton) {
            case "New York, NY":
...
When(
    /^User clicks on (Hallway Thermostat|Entryway Nest Protect) tab$/,
    { wrapperOptions: { retry: 2 } },
    tabName => {
        switch (tabName) {
...
When(
    /^User clicks on (Solar|Cleaner|Pool Heat|Pool Light|Waterfall|Spa Heat|Jets|Spa Light|Blower) button to change state to (On|Off) on Autelis page$/,
    (buttonName, state) => {
        switch (buttonName) {
            case "Solar":
                state === "On"
                    ? AutelisPage.clickSolarOnButton()
                    : AutelisPage.clickSolarOffButton();
                break;
            case "Cleaner":
                state === "On"
                    ? AutelisPage.clickCleanerOnButton()
                    : AutelisPage.clickCleanerOffButton();
                break;
            case "Pool Heat":
                state === "On"
                    ? AutelisPage.clickPoolHeatOnButton()
                    : AutelisPage.clickPoolHeatOffButton();
                break;
            case "Pool Light":
                state === "On"
                    ? AutelisPage.clickPoolLightOnButton()
                    : AutelisPage.clickPoolLightOffButton();
                break;
            case "Waterfall":
                state === "On"
                    ? AutelisPage.clickWaterfallOnButton()
                    : AutelisPage.clickWaterfallOffButton();
                break;
            case "Spa Heat":
                state === "On"
                    ? AutelisPage.clickSpaHeatOnButton()
                    : AutelisPage.clickSpaHeatOffButton();
                break;
            case "Jets":
            ...
            case "Spa Light":
                state === "On"
                    ? AutelisPage.clickSpaLightOnButton()
                    : AutelisPage.clickSpaLightOffButton();
                break;
            case "Blower":
                state === "On"
                    ? AutelisPage.clickBlowerOnButton()
                    : AutelisPage.clickBlowerOffButton();
                break;
        }
    }
);
...
When(/^User (decreases|increases) (Pool|Spa) heat on Autelis page$/, (action, type) => {
    switch (type) {
        case "Pool":
            action === "increases"
                ? AutelisPage.clickIncreasePoolHeat()
                : AutelisPage.clickDecreasePoolHeat();
            break;
        case "Spa":
            action === "increases"
                ? AutelisPage.clickIncreaseSpaHeat()
                : AutelisPage.clickDecreaseSpaHeat();
            break;
    }
});
